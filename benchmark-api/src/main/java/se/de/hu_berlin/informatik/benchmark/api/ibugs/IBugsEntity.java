...
public class IBugsEntity extends AbstractEntity {
	...
	private final String versionRoot; // looks like projectRoot/versions/28919/pre-fix/
	private final String targetRoot; // looks like versionRoot/org.aspectj/modules/aj-build/dist/tools/lib/
	private final String harnessTestResults; // looks like versionRoot/org.aspectj/modules/tests/harnessresults.xml
	...
	private IBugsEntity(String aFixedId, String aProject, String aProjectRoot, boolean aBuggy ) {
		...
		targetRoot = versionRoot + "org.aspectj/modules/aj-build/dist/tools/lib/"; // this is a constant because the ant file specifies it
	...
	public boolean compile(boolean executionMode) {
		...
		// lets save some calls and always build the tests together with the normal build
	...
	public boolean compileTests( boolean executionMode ) {
		...
		// this looks a bit strange but running the junittests creates files that store important data like
		// the classpath
		runJUnitTests();
	...
	public boolean runHarnessTests() {
		...
		// this only makes sense if the repository was built before and the target directory exists
		File targetDir = new File( targetRoot );
		if( !targetDir.exists() ) {
			Log.err( this, "The target directory does not exists making the execution of tests impossible: " + targetDir.getAbsolutePath() );
			
			// TODO decide if I should just trigger the clean building and proceed
			return false;
		}
	...
	/**
	 * Searches for the files that is created after the harness tests are run and reports the content.
	 * The file itself is not in a valid xml format and rather consists of several lines of output data.
	 * 
	 * This method is only public for debugging reasons and should not be used in a normal test szenario
	 * since it is already executed after the harness tests.
	 */
	public void parseHarnessTestResultFile() {
		...
		// the content is not really in an appealing format so the idea is to parse the start of each line
		// I think this should originally create a valid xml files but it somehow did not
		FileReader is = null;
	...
	public boolean runJUnitTests() {
		// TODO remove this information once I reworked the building of the test cases
		Log.out( this, "Running the junit test suites will most likely fail because of an ant error when building " + 
	...
	public List<Path> computeTestClasses(boolean executionMode) throws UnsupportedOperationException {
		...
		// the harness tests are all listed in the apjTests.xml and jimTests.xml files
		
		return null;
	}
	
	/**
	 * Just a temporary solution that collects the jars that are also mentioned in the ant script
	 * 
	...
	/**
	 * Finds all jars in the lib directory of the current version directory
	 * and returns them in class path format
	 * 
	 * @return a class path string containing all jars that will be needed
	 * or null if the library directory could not be found
	 */
	public String findAllCPJars() {
		String result = "";
		
		// as dirty as it gets but this is the simulation of the fileset copy from ant
		...
		// TODO I could parse all the .classpath files... maybe if something throws errors
		
		return result;
	}
	
	/**
	 * After building the files of a repository the jars that are relevant to monitoring
	 * are placed in a specified directory. This method returns the absolute path of all
	 * created jars.
	 * 
	 * @return A collection of absolute path names of relevant jars for monitoring or null if something is wrong
	 */
	...
	/**
	 * Searches for all .jar files in the given directory and adds them to the collection
	 * 
	 * @param aFile The root directory with some jars
	 * @param aJarsFound A collection storing all the paths to the found jars
	 */
	...
	/**
	 * Iterates over all entries in the collection and builds a classpath from it
	 * 
	 * @param aCollectionOfStrings A collection containing the absolute paths of files
