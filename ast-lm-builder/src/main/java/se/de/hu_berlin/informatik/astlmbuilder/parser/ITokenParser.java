...
public interface ITokenParser extends IModifierHandler, IOperatorHandler, ITypeHandler {
	...
	public default ThrowsStmt createThrowsStmt(String token, InformationWrapper info)throws IllegalArgumentException {
		List<String> memberData = parseAndCheckMembers(token, getKeyWordProvider().getThrowsStatement(), 0); 
		if (memberData == null) {
			return null;
		} else if (memberData.isEmpty()) { //token: $id
			return guessNodeFromKeyWord(ThrowsStmt.class, token, info.getCopy());
		}
		
		// boolean isStatic
		// BlockStmt body
		return new ThrowsStmt();
	}
	...
	// this may never be used because this class should be abstract anyway
	public default BodyStmt createBodyStmt(String token, InformationWrapper info) throws IllegalArgumentException {
		List<String> memberData = parseAndCheckMembers(token, getKeyWordProvider().getBodyStmt(), 0); 
		if (memberData == null) {
			return null;
		} else if (memberData.isEmpty()) { //token: $id
			return guessNodeFromKeyWord(BodyStmt.class, token, info.getCopy());
		}
	...
	// this may never be used
	public default ExtendsStmt createExtendsStmt(String token, InformationWrapper info)  throws IllegalArgumentException {
		List<String> memberData = parseAndCheckMembers(token, getKeyWordProvider().getExtendsStatement(), 1); 
		if (memberData == null) {
			return null;
		} else if (memberData.isEmpty()) { //token: $id
			return guessNodeFromKeyWord(ExtendsStmt.class, token, info.getCopy());
		}
		
		// List<ClassOrInterfaceType> extendsList
		return new ExtendsStmt();
//		return new ExtendsStmt( 
//				parseListFromToken(ClassOrInterfaceType.class, memberData.get(0), info.getCopy()),
//				null);
	}
	
	// this may never be used
	public default ImplementsStmt createImplementsStmt(String token, InformationWrapper info)  throws IllegalArgumentException {
		List<String> memberData = parseAndCheckMembers(token, getKeyWordProvider().getImplementsStatement(), 1); 
		if (memberData == null) {
			return null;
		} else if (memberData.isEmpty()) { //token: $id
			return guessNodeFromKeyWord(ImplementsStmt.class, token, info.getCopy());
		}
		
		// List<ClassOrInterfaceType> implementsList
		return new ImplementsStmt();
//				return new ExtendsStmt( 
//				parseListFromToken(ClassOrInterfaceType.class, memberData.get(0), info.getCopy()),
//				null);
	}
