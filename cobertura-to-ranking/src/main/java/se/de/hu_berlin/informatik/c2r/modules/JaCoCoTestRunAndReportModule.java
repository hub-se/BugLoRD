...
//	final private Arguments reportArguments;
	...
	public JaCoCoTestRunAndReportModule(final String testOutput, final String srcDir, String[] originalClasses,
		...
		this(testOutput, srcDir, originalClasses, false, null, 1, instrumentedClassPath, javaHome, useSeparateJVMalways);
	}

	public JaCoCoTestRunAndReportModule(final String testOutput, final String srcDir, String[] originalClasses,
			final boolean debugOutput, Long timeout, final int repeatCount,
			String instrumentedClassPath, final String javaHome, boolean useSeparateJVMalways) {
		this(testOutput, srcDir, originalClasses, debugOutput, timeout, repeatCount, 
	 ...
	 */
	@Override
	public JaCoCoReportWrapper processItem(final TestWrapper testWrapper, ProcessorSocket<TestWrapper, JaCoCoReportWrapper> socket) {
...
//	private ExecFileLoader runTestLocallyOrInJVM(final TestWrapper testWrapper, 
...
//	}
	...
	private ExecFileLoader runTestLocally(final TestWrapper testWrapper, 
		...
		if (testStatistics.couldBeFinished()) {
			try {
				...
				Log.err(this, e, testWrapper + ": Could not request execution data after running test no. " + testCounter + ".");
				testStatistics.addStatisticsElement(StatisticsData.ERROR_MSG, 
	...
	private ExecFileLoader dump(final InetAddress address, final int port)
			throws IOException {
	...
	private Socket tryConnect(final InetAddress address, final int port)
		...
		while (true) {
			try {
				Log.out(this, "Connecting to %s:%s.", address, Integer.valueOf(port));
	...
	private ExecFileLoader runTestInNewJVM(TestWrapper testWrapper, TestStatistics testStatistics) {
		...
		if (testStatistics.couldBeFinished()) {
			try {
				...
				loader = UNDEFINED_COVERAGE_DUMMY;
				Log.err(this, testWrapper + ": Could not request execution data after running test no. " + testCounter + ".");
				testStatistics.addStatisticsElement(StatisticsData.ERROR_MSG, 
						testWrapper + ": Could not request execution data after running test no. " + testCounter + ".");
			}
		} else {
	...
	private JaCoCoReportWrapper generateReport(final TestWrapper testWrapper, 
			TestStatistics testStatistics, ExecFileLoader execFileLoader) {
		//generate the report
...
//		}
		...
		return new JaCoCoReportWrapper(bundle, 
	...
	private void writeReports(final IBundleCoverage bundle,
			...
			throws IOException {
		Log.out(this, "Analyzing %s classes.%n",
...
//			visitors.add(formatter.createVisitor(new FileOutputStream("test.xml")));
//		}
//
//		if (csv != null) {
//			final CSVFormatter formatter = new CSVFormatter();
//			visitors.add(formatter.createVisitor(new FileOutputStream("test.csv")));
//		}
//
//		if (html != null) {
			final HTMLFormatter formatter = new HTMLFormatter();
			visitors.add(
					formatter.createVisitor(new FileMultiReportOutput(new File("test_html"))));
//		}
