...
public class RankingModule<T> extends AbstractProcessor<ISpectra<T, ?>, ISpectra<T, ?>> {
     ...
     * @param localizers a list of Cobertura localizer identifiers
     */
...
//			Log.out(this, "...calculating " + className + " ranking.");
     ...
     *
     * @param spectra   Cobertura line spectra
     * @param localizer provides specific SBFL formulae
    ...
    private void generateRanking(final ISpectra<T, ?> spectra,
        ...
        try {
            ...
            ranking.save(outputdir + File.separator + subfolder + File.separator + "ranking2.rnk");
            System.out.println("------- ranking saved again -------"); //PT
            System.out.println(ranking.getBestRankingElement()); //PT
            System.out.println(ranking.getBestRankingValue()); //PT
        } catch (IOException e) {
