...
public class CoberturaSpectraGenerationFactory extends AbstractSpectraGenerationFactory<ProjectData, CoberturaReportWrapper> {
	...
	public AbstractInstrumenter getInstrumenter(Path projectDir, String outputDir,
		...
		return new CoberturaInstrumenter(projectDir, outputDir, testClassPath,
				(String[]) pathsToBinaries, coberturaDataFile);
	}
	...
	public AbstractRunSingleTestAndReportModule<ProjectData, CoberturaReportWrapper> getTestRunnerModule(
		...
		return new CoberturaRunSingleTestAndReportModule(coberturaDataFile.toPath().toAbsolutePath(), 
				outputDir, projectDir.toFile(), srcDir.toString(),
				options.hasOption(CmdOptions.FULL_SPECTRA), RunTestsAndGenSpectraProcessor.TEST_DEBUG_OUTPUT,
				...
				RunTestsAndGenSpectraProcessor.class.getResource("/testrunner.jar").getPath(), options.hasOption(CmdOptions.SEPARATE_JVM),
				options.hasOption(CmdOptions.JAVA7), options.getOptionValueAsInt(CmdOptions.MAX_ERRORS, 0), 
				options.getOptionValues(CmdOptions.FAILING_TESTS), statisticsContainer,
