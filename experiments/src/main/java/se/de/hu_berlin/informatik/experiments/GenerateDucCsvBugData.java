...
public class GenerateDucCsvBugData {
    ...
    public static void main(String[] args) {
        ...
        OptionParser options = OptionParser.getOptions("GenerateCsvBugDataFiles", true, se.de.hu_berlin.informatik.experiments.defects4j.GenerateCsvBugDataFiles.CmdOptions.class, args);

        // get the output path (does not need to exist)
        Path output = options.isDirectory(GenerateDucCsvBugData.CmdOptions.OUTPUT, false);

        String suffix = options.getOptionValue(GenerateDucCsvBugData.CmdOptions.SUFFIX, null);

        String[] localizers = options.getOptionValues(GenerateDucCsvBugData.CmdOptions.LOCALIZERS);
        if (localizers == null) {
            localizers = BugLoRD.getValueOf(BugLoRD.BugLoRDProperties.LOCALIZERS).split(" ");
        }

        if (localizers.length < 1) {
            Log.abort(GenerateDucCsvBugData.class, "No localizers given.");
        }
        ...
        // bug size data
        {
            PipeLinker linker = new PipeLinker().append(
                    new ThreadedProcessor<>(options.getNumberOfThreads(),
                            new GenerateDucCsvBugData.RankingLOCProcessor(suffix, localizers[0])),
        ...
        for (String localizer : localizers) {
            Log.out(GenerateDucCsvBugData.class, "Processing %s.", localizer);
            PipeLinker linker2 = new PipeLinker().append(
                    new ThreadedProcessor<>(options.getNumberOfThreads(),
                            new GenerateDucCsvBugData.GenStatisticsProcessor(suffix, localizer)),
        ...
        Log.out(GenerateDucCsvBugData.class, "All done!");
    ...
    private static class GenStatisticsProcessor extends AbstractProcessor<BuggyFixedEntity<?>, Pair<String, String[]>> {
        ...
        public Pair<String, String[]> processItem(BuggyFixedEntity<?> entity, ProcessorSocket<BuggyFixedEntity<?>, Pair<String, String[]>> socket) {
            Log.out(GenerateDucCsvBugData.class, "Processing %s.", entity);
            Entity bug = entity.getBuggyVersion();

            Map<String, List<Modification>> changeInformation = entity.loadChangesFromFile();

            Ranking<ProgramBranch> ranking = RankingDuc.getRanking(bug, suffix, rankingIdentifier);
            if (ranking == null) {
                Log.abort(this, "Found no ranking with identifier '%s'.", rankingIdentifier);
            }

            MarkedRanking<ProgramBranch, List<Modification>> markedRanking = new MarkedRanking<>(ranking);
            ...
            int count = 0;
            for (ProgramBranch changedElement : markedRanking.getMarkedElements()) {
                String[] line = new String[15];
                RankingMetric<ProgramBranch> metric = Objects.requireNonNull(ranking).getRankingMetrics(changedElement);
                RankingDuc.ProgramBranchRankingMetrics scbMetric = RankingDuc.getProgramBranchRankingMetrics(ranking, changedElement);
    ...
    private static class RankingLOCProcessor extends AbstractProcessor<BuggyFixedEntity<?>, String> {
        ...
        public String processItem(BuggyFixedEntity<?> entity, ProcessorSocket<BuggyFixedEntity<?>, String> socket) {
            Log.out(GenerateDucCsvBugData.class, "Processing %s for general data.", entity);
            Entity bug = entity.getBuggyVersion();
